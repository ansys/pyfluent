...
class Transcation:
    ...
    def __init__(self, fileSession):
        self._surface_transcations = []
        self._scalar_field_transcations = []
    ...
    def add_surfaces_request(
        ...
        for surface_id in surface_ids:
            self._surface_transcations.append(Transcation._SurfaceTransaction(surface_id, provide_vertices, provide_faces)) 
    ...
    def add_scalar_fields_request(
        ...
        for surface_id in surface_ids:
            self._scalar_field_transcations.append(Transcation._ScalarFieldTransaction(field_name, surface_ids))    

    def get_fields(self):
        mesh = self._fileSession._case_file.get_mesh()
        field_data = {}
        
        scalar_field_tag=(("type", "scalar-field"),
                        (
                            "dataLocation",
        ...
        for transcation in self._scalar_field_transcations:
            if (scalar_field_tag) not in field_data:
                field_data[scalar_field_tag] = {}
            field_data_surface =  field_data[scalar_field_tag]    
            for surface_id in transcation.surface_ids:                   
                field_data_surface[surface_id] = {}
                field_data_surface[surface_id][transcation.field_name] = self._fileSession._data_file.get_face_data("phase-1",transcation.field_name,  surface_id) 
                

        for transcation in self._surface_transcations:
            if (("type", "surface-data"),) not in field_data:
                field_data[(("type", "surface-data"),)] = {}
            field_data_surface =  field_data[(("type", "surface-data"),)]   
            field_data_surface[transcation.surface_id] = {}
            field_data_surface[transcation.surface_id]["faces"] = mesh.get_connectivity(transcation.surface_id) 
            field_data_surface[transcation.surface_id]["vertices"] = mesh.get_vertices(transcation.surface_id)             
        return field_data 
...
class FileFieldInfo:
    ...
    def get_scalar_field_range(
        ...
        if not surface_ids:
            surface_ids = self._fileSession._case_file.get_mesh().get_surface_ids()
        for surface_id in surface_ids:                    
            data = self._fileSession._data_file.get_face_data("phase-1", field, surface_id)
            if len(data)==0:
                continue 
    ...
    def get_scalar_fields_info(self) :
        ...
        Dict
        """
        phases = self._fileSession._data_file.get_phases()
        ...
        return {
            ...
            for phase in phases for face_variable in self._fileSession._data_file.get_face_variables(phase)
        }        
