...
class GraphicsWidget(metaclass=SingletonMeta):
    ...
    def create_callback(self):
            ...
            for name, value in obj.__dict__.items():
                ...
                if value.__class__.__class__.__name__ in (
                    ...
                    if value.__class__.__class__.__name__ == "PyLocalPropertyMeta":
                        ...
                        store_all_widgets(obj_type, value, parent+"/"+name, parent_visible and visible)
        ...
        def update_stored_widgets(value, session_id=None):
            if value is not None:
                update_object(value, session_id)
                self._all_widgets[value] ={}
                store_all_widgets(value, self._object)
        ...
        def on_value_changed(values, session_id, graphics_selection,):              
        ...
        def refresh_widgets(i1, session_id, graphics_selector): 
            print('show hide', i1, session_id, graphics_selector)
            if graphics_selector is None:
                raise PreventUpdate            
            update_stored_widgets(graphics_selector, session_id)
            return list(self._all_widgets[graphics_selector].values())
    ...
    def get_widget(self, obj, type, name, obj_type, parent, visible, unique_name, attributes):
        ...
        if str(type) == "<class 'str'>":
            if attributes and "allowed_values" in attributes:
                ...
                widget = dcc.Input(id={'type': 'graphics-widget', 'index': unique_name}, type="text", value=obj())
            ...
            if attributes and "range" in attributes:
                ...
                widget = dcc.Input(id={'type': 'graphics-widget', 'index': unique_name}, type="number", value=obj())
