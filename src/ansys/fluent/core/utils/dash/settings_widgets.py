...
class SettingsWidgetBase():
    ...
    def get_object_and_static_info(self, object_type, connection_id, session_id, object_id=None):
        ...
        if object_type is not None:   
            ...
            static_info = self.SessionsManager(self._app, connection_id, session_id).static_info            
            obj = self.SessionsManager(self._app, connection_id, session_id).settings_root
            for path in path_list:
                obj = getattr(obj, path)
                static_info = static_info['children'][path]                
    ...
    def create_callback(self):
        def store_all_widgets(obj, si_info, state, parent=""):
            ...
            for name, value in si_info['children'].items():
                print('store_all_widgets', name, si_info)
                #si_info_child = si_info['children'][name] 
                si_info_child =  value  
        ...
        def update_stored_widgets(object_type, connection_id, session_id):
            obj, static_info = self.get_object_and_static_info(object_type, connection_id, session_id)
            self._all_widgets = {}            
        ...
        @self._app.callback(
            ...
            Input({"type": f"settings-widget-{self._id}", "index": ALL}, "value"),
        ...
        def refresh_widgets(_, connection_id, object_type, session_id):
            print("show hide settings", _, connection_id, session_id, object_type)
    ...
    def get_widget(
        ...
        print('get_widget', obj, name, path, static_info)
...
class SettingsWidget(SettingsWidgetBase):
    ...
    def __init__(self, app, connection_id, session_id, SessionsManager):
        ...
        window_state = SettingsWidget._state.get(
            id
        )           
        if not window_state:  
            SettingsWidget._state[
                id
            ] = self.__dict__
            self._type_to_path ={}
            self._type_to_path["Viscous"]= "setup/models/viscous"            
        ...
        return dbc.Row(
            [
                ...
                dbc.Col(
                    ...
                    + [
                        html.Div(
                            html.Div(id=f"settings-card-body-{self._id}",
