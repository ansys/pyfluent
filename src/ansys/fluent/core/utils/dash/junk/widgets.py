    ...
    def create_callback(self):
        def update_stored_widgets(value):
            if value is not None:
                ...
                ] = self.get_button("display", self.get_unique_name("display"))
        ...
        @self._app.callback(
            [
                Output(f"{graphics_type}-graphics-card", "style")
                for graphics_type in self._graphics
            ],
        ...
        def show_graphics_object(value, session_id):
            ...
            return [
                {"display": "block"} if graphics_type == value else {"display": "none"}
                for graphics_type in self._graphics
            ]

        print(
            [
                list(self._all_widgets[graphics_type].keys())[-1]
                for graphics_type in self._graphics
            ]
        )
        ...
        @self._app.callback(
            ...
            [
                Input(list(self._all_widgets[graphics_type].keys())[-1], "n_clicks")
                for graphics_type in self._graphics
            ],
        )
