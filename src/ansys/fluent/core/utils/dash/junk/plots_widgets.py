...
class PlotsWidget(metaclass=SingletonMeta):
    ...
    def layout(self, plots_selector_value=None):
        ...
        return dbc.Row(
            [
                dbc.Col(dcc.Graph(id="plot-viewer", style={"height": 900})),
                dbc.Col(
                    [
                        html.Div(
                            [
    ...
    def update_object(self, value, session_id=None):
        if value is not None:
            if value == "XYPlot":
                self._object = plots_session1.XYPlots[
                    "dummy-xyplot" + session_id if session_id else ""
                ]
    ...
    def create_callback(self):
        def update_stored_widgets(value):
            if value is not None:
                ...
                self._all_widgets[value][
                    self.get_unique_name("plot")
                ] = self.get_button("plot", self.get_unique_name("plot"))
        ...
        @self._app.callback(
            [
                Output(f"{plot_type}-graphics-card", "style")
                for plot_type in self._plots
            ],
        ...
        def show_graphics_object(value, session_id):
            ...
            return [
                {"display": "block"} if plot_type == value else {"display": "none"}
                for plot_type in self._plots
            ]
        ...
        @self._app.callback(
            ...
            [
                Input(list(self._all_widgets[plot_type].keys())[-1], "n_clicks")
                for plot_type in self._plots
            ],
        )
        def plot_graph(*args):
        ...
        for value in self._plots:
            ...
            def callback(*args):
                ...
                widget_values = []
                for widget_value in [
                    widgets_value_map[name]
                    if name in visible_widgets
                    else self._widget_value_map[name]
                    for name in list(self._all_widgets[value].keys())[:-1]
                ]:
            ...
            if value.__class__.__class__.__name__ in (
                ...
                if value.__class__.__class__.__name__ == "PyLocalPropertyMeta":
                    ...
                    self.store_all_widgets(
                        obj_type, value, parent + "/" + name, parent_visible and visible
                    )
    ...
    def get_button(self, name, unique_name):
        ...
        self._button_widget = dbc.Button(
            self.get_label(name), id=unique_name, n_clicks=0
        )

        return self._button_widget

    def get_widget(
        self, obj, type, name, obj_type, parent, visible, unique_name, attributes
    ):
