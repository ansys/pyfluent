...
.. code-block:: python
    ...
    >>> case_file_path = examples.download_file("Static_Mixer_Parameters.cas.h5", "pyfluent/static_mixer", return_only_file_name=False)

    >>> reader = CaseFile(case_file_path=case_file_path) # Instantiate a CaseFile class
...
class SettingsFile(RPVarProcessor):
    ...
    def __init__(self, settings_file_path: Optional[str] = None) -> None:
        ...
        Parameters
        ----------
        settings_file_path : Optional[str]
        ...
        if settings_file_path:
            try:
                ...
                raise FileNotFoundError(
                    f"The settings file {settings_file_path} cannot be found."
                ) from e

            except OSError as e:
                raise OSError(
                    f"Error while reading settings file {settings_file_path}"
                ) from e

            except Exception as e:
                raise RuntimeError(
                    f"Could not read settings file {settings_file_path}"
                ) from e
    ...
    def __init__(
        ...
        if project_file_path:
            if Path(project_file_path).suffix in [".flprj", ".flprz"]:
                ...
                case_file_path = Path(
                    project_dir + _get_case_file_path_from_flprj(project_file_path)
                )
            else:
                raise FileNotFoundError(
        ...
        try:
            if Path(case_file_path).match("*.cas.h5"):
                _file = h5py.File(case_file_path)
                settings = _file["settings"]
                rpvars = settings["Rampant Variables"][0]
                rp_vars_str = rpvars.decode()
            elif Path(case_file_path).match("*.cas"):
                with open(case_file_path, "rb") as _file:
                    rp_vars_str = _file.read()
                rp_vars_str = _get_processed_string(rp_vars_str)
            elif Path(case_file_path).match("*.cas.gz"):
                with gzip.open(case_file_path, "rb") as _file:
                    rp_vars_str = _file.read()
                rp_vars_str = _get_processed_string(rp_vars_str)
            else:
                error_message = (
            ...
            raise FileNotFoundError(
                f"The case file {case_file_path} cannot be found."
            ) from e

        except OSError as e:
            raise OSError(f"Error while reading case file {case_file_path}") from e

        except Exception as e:
            raise RuntimeError(f"Could not read case file {case_file_path}") from e
...
def _get_case_file_path_from_flprj(flprj_file):
