...
class _DataModelClient:
    def __init__(self, rules):
        self._rules = rules
        self._channel = None
        self._data_model_service = None

    def run_server(self, batch_file_path: Union[str, Path]):
        run_command = str(batch_file_path) + " " + self._rules
    ...
    def set_state(self, path: str, state: str):
        py_menu = PyMenu(self._data_model_service, rules=self._rules, path=[(path, "")])
