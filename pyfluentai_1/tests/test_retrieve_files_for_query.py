...
def retrieve_filenames(retriever, query):
    ...
    return [
        node.metadata.get("file_name") or node.metadata.get("source_file")
        for node in results
    ]
...
def retriever(request):
    ...
    index = request.param(
        doc_dir=DOC_DIR, persist_dir=persist_dir
    ).get_index()  # use a per-indexer persisted cache; setup/teardown cleans it for test isolation
...
def test_retriever_returns_results_for_field_data_query(retriever):
    results = retriever.retrieve(
        "What is the difference between field data and solution data?"
    )
    assert len(results) > 0
...
def test_physical_units(retriever):
    ...
    if "flunits.py" not in filenames:
        if retriever.indexer_name == "SectionIndexer":
            ...
            assert (
                False
            ), f"flunits.py should be in {filenames} for {retriever.indexer_name}"
    else:
