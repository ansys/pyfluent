...
Pre/post mode
~~~~~~~~~~~~~
Run Ansys Fluent in pre/post mode. In PyFluent, this mode maps to the ``Solver`` session type, but when Fluent is running in ``PRE_POST`` mode, solve commands are not available.
