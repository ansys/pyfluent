...
cold_inlet = bc.velocity_inlet["cold-inlet"]
...
cz.fluid["elbow-fluid"].laminar.set_state(True)
```
...
```{python}
#| eval: false
vs = solver.settings.setup.models.viscous
from pprint import pprint
...
```{python}
#| eval: false
rn = solver.settings.setup.models.radiation
from pprint import pprint
...
```{python}
#| eval: false
pvc = solver.settings.solution.controls.p_v_controls
...
solver.fields.reduction.area(
  locations=[bc.velocity_inlet["inlet1"]]
)
solver.fields.reduction.area(
    locations=["inlet1"], 
    ctxt=solver)
