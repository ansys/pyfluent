...
data. It has two APIs.

- An API to get multiple fields in a single request 
- An API to get one field per request 
...
Surface request 
~~~~~~~~~~~~~~~
The response contains any of the following fields, depending on the request arguments:
...
~~~~~~~~~~~~~~~~~~~~
The response contains a single field with the same name as the scalar field name passed in the request.

Vector field request
~~~~~~~~~~~~~~~~~~~~
The response contain two fields:
...
---------------------
You can receive one field for each request. There is a separeate API for each type of field (surface, scalar, 
or vector):
...
For a surface or scalar field, the response contains a dictionary of surface ID and a numpy array of 
the requested field. 

surface_id [int] -> field[np.array]
  
For a vector field, the response is a dictionary of surface IDs and a tuple of a numpy array of ``vector field`` 
and ``vector scale``. 
...
IDs. Thus, a response can contain surface ID as key of the returned dictionary. 
