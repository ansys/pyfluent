...
def _test_locn_extraction(solver1, solver2):
    s1_boundary_conditions = solver1.setup.boundary_conditions
    s2_boundary_conditions = solver2.setup.boundary_conditions
    locns = _locn_names_and_objs(["inlet1"])
    assert locns == [["inlet1", ["inlet1"]]]

    all_bcs = s1_boundary_conditions
    ...
    all_bcs = s1_boundary_conditions
    all_bcs2 = s2_boundary_conditions
...
def _test_area_average(solver):
    solver.solution.initialization.hybrid_initialize()
    s_named_express = solver.setup.named_expressions
    s_named_express["test_expr_1"] = {}
    s_named_express["test_expr_1"].definition = "AreaAve(AbsolutePressure, ['inlet1'])"
    expr_val = s_named_express["test_expr_1"].get_value()
    ...
    assert val == expr_val
    s_named_express.pop(key="test_expr_1")
...
def _test_min(solver1, solver2):
    ...
    if solver1.get_fluent_version() < "24.1.0":
        solver1_vmag = s1_boundary_conditions["inlet1"].vmag
        solver2_vmag = s2_boundary_conditions["inlet1"].vmag
    else:
        solver1_vmag = s1_boundary_conditions["inlet1"].momentum.velocity
        solver2_vmag = s2_boundary_conditions["inlet1"].momentum.velocity
    ...
    s1_named_expr = s1_named_expr
    s1_named_expr["test_expr_1"] = {}
    test_expr1 = s1_named_expr["test_expr_1"]
    test_expr1.definition = "sqrt(VelocityMagnitude)"
    s1_named_expr["test_expr_2"] = {}
    test_expr2 = s1_named_expr["test_expr_2"]
    ...
    result = solver1.reduction.minimum(
        ...
        locations=[
            s1_boundary_conditions["outlet"],
            s2_boundary_conditions["outlet"],
        ],
    )
    # assert result == expected_result
    s1_named_expr.pop(key="test_expr_1")
    s1_named_expr.pop(key="test_expr_2")
...
def _test_count(solver):
    ...
    s_named_express["test_expr_1"] = {}
    s_named_express["test_expr_1"].definition = "Count(['inlet1'])"
    expr_val_1 = s_named_express["test_expr_1"].get_value()
    s_named_express["test_expr_1"].definition = "Count(['inlet2'])"
    expr_val_2 = s_named_express["test_expr_1"].get_value()
    s_named_express["test_expr_1"].definition = "Count(['inlet1', 'inlet2'])"
    expr_val_3 = s_named_express["test_expr_1"].get_value()
...
def _test_centroid(solver):
    ...
    s_velocity_inlet = solver.setup.boundary_conditions.velocity_inlet
    s_named_express["test_expr_1"] = {}
    s_named_express["test_expr_1"].definition = "Centroid(['inlet1'])"
    expr_val_1 = s_named_express["test_expr_1"].get_value()
    s_named_express["test_expr_1"].definition = "Centroid(['inlet2'])"
    expr_val_2 = s_named_express["test_expr_1"].get_value()
    s_named_express["test_expr_1"].definition = "Centroid(['inlet1', 'inlet2'])"
    expr_val_3 = s_named_express["test_expr_1"].get_value()
    red_val_1 = solver.reduction.centroid(locations=[s_velocity_inlet["inlet1"]])
    red_val_2 = solver.reduction.centroid(locations=[s_velocity_inlet["inlet2"]])
    red_val_3 = solver.reduction.centroid(locations=[s_velocity_inlet])
...
def _test_area_integrated_average(solver1, solver2):
    ...
    s1_boundary_conditions = solver1.setup.boundary_conditions
    s2_boundary_conditions = solver2.setup.boundary_conditions
    s1_named_expr = solver1.setup.named_expressions
    s2_named_expr = solver2.setup.named_expressions

    s1_named_expr["test_expr_1"] = {}
    s1_named_expr["test_expr_1"].definition = "AreaInt(AbsolutePressure, ['inlet1'])"
    expr_val_1 = s1_named_expr["test_expr_1"].get_value()

    s1_named_expr["test_expr_1"].definition = "AreaInt(AbsolutePressure, ['inlet2'])"
    expr_val_2 = s1_named_expr["test_expr_1"].get_value()
    s1_named_expr[
        "test_expr_1"
    ].definition = "AreaInt(AbsolutePressure, ['inlet1', 'inlet2'])"
    expr_val_3 = s1_named_expr["test_expr_1"].get_value()

    assert expr_val_3 - (expr_val_1 + expr_val_2) <= 0.000000001

    red_val_1 = solver1.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s1_boundary_conditions.velocity_inlet["inlet1"]],
    )
    red_val_2 = solver1.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s1_boundary_conditions.velocity_inlet["inlet2"]],
    )
    red_val_3 = solver1.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s1_boundary_conditions.velocity_inlet],
    )
    ...
    s2_named_expr["test_expr_1"] = {}
    s2_named_expr["test_expr_1"].definition = "AreaInt(AbsolutePressure, ['inlet1'])"
    expr_val_4 = s2_named_expr["test_expr_1"].get_value()

    s2_named_expr["test_expr_1"].definition = "AreaInt(AbsolutePressure, ['inlet2'])"
    expr_val_5 = s2_named_expr["test_expr_1"].get_value()
    s2_named_expr[
        "test_expr_1"
    ].definition = "AreaInt(AbsolutePressure, ['inlet1', 'inlet2'])"
    expr_val_6 = s2_named_expr["test_expr_1"].get_value()

    assert expr_val_6 - (expr_val_4 + expr_val_5) <= 0.000000001

    red_val_4 = solver2.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s2_boundary_conditions.velocity_inlet["inlet1"]],
    )
    red_val_5 = solver2.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s2_boundary_conditions.velocity_inlet["inlet2"]],
    )
    red_val_6 = solver2.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[s2_boundary_conditions.velocity_inlet],
    )
    ...
    red_val_7 = solver2.reduction.area_integral(
        expression="AbsolutePressure",
        locations=[
            s1_boundary_conditions.velocity_inlet,
            s2_boundary_conditions.velocity_inlet,
        ],
    )
...
def _test_force(solver):
    ...
    s_named_express = solver.setup.named_expressions
    s_named_express["test_expr_1"] = {}
    s_named_express["test_expr_1"].definition = "Force(['wall'])"
    expr_val_1 = s_named_express["test_expr_1"].get_value()
...
def _test_moment(solver):
    ...
    s_named_express["test_expr_1"] = {}
    s_named_express["test_expr_1"].definition = "Moment(Force(['wall']),['wall'])"
    expr_val_1 = s_named_express["test_expr_1"].get_value()

    s_named_express["test_expr_1"].definition = "Moment(['inlet1'],['wall'])"
    expr_val_2 = s_named_express["test_expr_1"].get_value()
