...
from ansys.fluent.addons.parametric_old import (
    DesignPoint,
    DesignPointStatus,
    DesignPointTable,
    ParametricStudy
...
def test_create_user_defined_design_point():
    dp = DesignPoint("DP1", base_design_point=make_simple_base_design_point())
    assert dp.name == "DP1"
    assert dp.inputs == {"x": 2}
    assert dp.outputs == {"y": 0}
    assert dp.status == DesignPointStatus.OUT_OF_DATE
...
def test_start_and_end_design_point_update():
    ...
    dp1 = DesignPoint("DP1", base_design_point=base_dp)
...
def test_run_parametric_study():
    ...
    study = ParametricStudy(
        base_design_point_name="xxx", launcher=XTimes(multiplier)
    )
    inputs = [0, 3, 8, -5]
...
def test_run_parametric_study_and_block():
    ...
    for i in range(len(inputs)):
        status = (
            DesignPointStatus.BLOCKED
            if i % 2
            else DesignPointStatus.OUT_OF_DATE
        )
        assert study.design_point(i).status == status
    study.update_all()
    for i in range(len(inputs)):
        status = (
            DesignPointStatus.BLOCKED if i % 2 else DesignPointStatus.UPDATED
        )
        assert study.design_point(i).status == status
        assert (
            study.design_point(i).outputs["y"]
