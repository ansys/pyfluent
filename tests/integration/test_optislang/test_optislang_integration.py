...
def test_simple_solve(load_mixing_elbow_param_case_dat):
    ...
    inputs_table["inlet2_temp"] = 600
    solver_session.settings.file.write_case(file_name="design_elbow_param.cas.h5")
    ...
    inputs_table = InputParameters(solver_session)
    assert input_parameters[0] == inputs_table["inlet2_temp"]

    outputs_table = OutputParameters(solver_session)
    outputs_table_units = outputs_table["outlet_temp-op"].split(" ")[1].strip("[]")
    outputs_table_value = float(outputs_table["outlet_temp-op"].split(" ")[0])
    output_parameters2 = solver_session.settings.parameters.output_parameters.list()
    output_parameters2_value = output_parameters2["outlet_temp-op"][0]
    assert output_parameters2_value == outputs_table_value
    assert output_parameters[1] == outputs_table_units
    solver_session.exit()
